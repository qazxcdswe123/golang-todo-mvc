...
// SearchTodos search todos with the given query
func SearchTodos(dest interface{}, userIden interface{}, query string) *gorm.DB {
	return database.DB.Model(&Todo{}).Find(dest, "user = ? AND task LIKE ?", userIden, "%"+query+"%")
}
